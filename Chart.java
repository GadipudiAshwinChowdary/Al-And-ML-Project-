...
private static CategoryDataset createDataset(){
	...
	for(Map.Entry<String,POW> pow : IndustrialManager.pow.entrySet()){
		...
		if(p.getDoubleSpending() > 0){
			localDefaultCategoryDataset.addValue(p.getDoubleSpending(),p.getNode()+" Abnormal Weight",p.getNode()+" Abnormal Weight");
		}
	}
...
private static JFreeChart createChart(CategoryDataset paramCategoryDataset){
	JFreeChart localJFreeChart = ChartFactory.createBarChart(title,"Technique Name", "Time (sec)", paramCategoryDataset, PlotOrientation.VERTICAL, true, true, false);
